...
export default function App(){
    ...
    const renderMoeda = ({ item }) => {

        const variacaoPositiva = Number(item.pctChange) >= 0

        return (
            <View style={styles.itemLista}>
                <View style={styles.conteudoItem}>
                    <MaterialCommunityIcons
                        name={variacaoPositiva ? "trending-up" : "trending-down"}
                        size={20}
                        color={variacaoPositiva ? "#16A34A" : "#EF4444"}
                    />
